...
Phase 5 requires: index.html (root composition) + fresh Phase-4 stamp; Phase 4's exit gate includes
  ...
  the `media-use` audio engine (AUDIO_ENGINE); the local ElevenLabs/Freesound scripts are a
  deprecated fallback. Neither path skips the exact-track confirmation, the caption review, or
  render approval.
...
Phase 3: DESIGN ──── Phase 4: PRODUCTION ──── Phase 5: AUDIO &amp; RENDER
  │                    │                        │
  ├ hyperframes skill  ├ HyperFrames root html  ├ media-use audio engine
  ├ DESIGN.md          ├ Sub-comp wiring        ├ Reviewed captions
  ├ Scene templates    ├ Seam ledger + stamp    ├ Confirmed track + mix
  └ Brand & motion     └ seam gate + lint/check └ npx hyperframes render
```
...
user confirms that exact track or an explicit none before any mix — the `scripts/caption_gen.py`
`draft` → `approve` → `finalize` → `validate` review contract over the caption data, the verified
...
- [scripts/generate_voiceover.py](scripts/generate_voiceover.py) — voiceover-section assembly for both audio paths, plus the ElevenLabs/Whisper **acquisition fallback** (deprecated, removed in M6, when the `media-use` engine is unavailable); `scripts/search_music.py` is the Freesound counterpart of that fallback
