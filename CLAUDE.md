...
- **Never ship a partly wired `<video>` in a clip scene.** The runtime frame-syncs only videos carrying `data-start`. `lint` catches a *bare* one — `media_missing_data_start` and `media_missing_id` are both **errors** on the pinned CLI, and `check` skips the browser when lint errors. What no rule covers is a *partly* wired video: there is no lint code for `data-media-start` or for a media element's `data-track-index`, so a video with `id` and `data-start` but the wrong offset plays the wrong footage with every gate green. The whole contract is `id` + `data-start="0"` + `data-duration` (loader's crossfade-extended window, not the bare clip length) + `data-media-start` (storyboard `Clip in`) + `data-track-index="0"` — see `workflows/phase-3-design.md` § Clip scene.

## Common edits

- **Add a voice** → the voice contract is stated in **three** places and all three must agree:
  the `## ElevenLabs Voice IDs` table in `SKILL.md`, the `## Voices` table in `README.md`, and
  the picker the user actually answers in `workflows/phase-1-storytelling.md` (plus its worked
  `elevenlabs:<name>:<id>` example). A voice added to the tables but not the picker can never be
  chosen; one added to the picker but not the tables records an id nothing documents.
  `test/unit/test_voice_parity.py` enforces the three-way agreement. Those IDs are the
  user-facing contract that the brief's `voice` field spells out and the `media-use` engine's
  ElevenLabs route synthesizes; no script in this repo resolves them any more.
...
- **Change the recorded-flow replay contract** → the coupled surfaces are: the format/pacing/ledger
  law in `patterns/recorded-flow-capture.md`; the take recipe and Step 2.1b consent in
  `workflows/phase-2-capture.md`; the `recording`/`recording_steps` rows in `templates/storyboard.md`
  § Capture and clip keys (scraped by `validate_brief.py load_frame_vocabulary` and by
  `test_director_keys.py` — template rows and the workflow lines that write the bullets must land
  together); the continue/jump predicates in `SKILL.md` (`replay_flow.py check`); the pointer-track
  consumption in `workflows/phase-3-design.md` § Clip scene; and the attached-session carve-out in
  `patterns/authenticated-browser-capture.md`. Pacing numbers live only in `replay_flow.py`'s
  profile block (test-asserted). The storyboard legacy mapping table is deliberately untouched:
  these keys never had a legacy spelling, so there is nothing to round-trip. The two screencast
  behavior probes (`SCREENCAST_POINTER_ABSENCE`, `SCREENCAST_CONCURRENT_CAPTURE`) live in
  `compat/ecosystem.md` § Behavior probes.
...
- **Change where the skill can be installed** (`$SKILL_HOMES`) → `SKILL.md` § Runtime Compatibility holds the canonical search list. The `SKILL_HOMES="…"` line appears at **seven** sites; **six** of them are markdown blocks that also carry the `SKILL_ROOT=…` bootstrap and the zsh portability guard — the canonical block itself, the `SKILL.md` prereq probe, the `SKILL_DIR` resolver of `workflows/phase-3-design.md`, **both** resolvers of `workflows/phase-4-production.md` (`DOCTRINE_SKILL_DIR`, Step 4.5; `ANIM_SKILL_DIR`, Step 4.7), and the `SKILL_DIR` + `MEDIA_SKILL_DIR` block of `workflows/phase-5-audio.md`. The seventh is `scripts/check_requirements.sh`, which shares only the `SKILL_HOMES` line: it carries a bash shebang and every call site invokes it under bash, so it word-splits correctly and **must not** be given the guard (`setopt` is not a bash builtin). Shell state can't cross the agent's separate bash calls, so the list is re-stated at each bootstrap point rather than sourced. The `|` delimiter plus `IFS='|'` preserves skill paths containing spaces. **The guard line is load-bearing, not decoration:** zsh neither word-splits an unquoted `$SKILL_HOMES` nor tolerates an unmatched `"$home"/*/` glob, so a resolver without it silently resolves to empty and every workflow reads that as "companion skill not installed". Keep all `SKILL_HOMES` lines byte-identical, and keep the guard identical with them; verify with:
  ```bash
